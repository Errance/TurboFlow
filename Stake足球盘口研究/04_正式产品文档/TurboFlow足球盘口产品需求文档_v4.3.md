...
| 用户目标 | 用户能理解比赛是否可下注、盘口在判断什么、为什么某些盘口不能组合 | 盘口说明、冲突提示、状态提示、客服解释口径 |
...
| 熟练足彩用户 | 想比较多个盘口 | 看懂每类盘口和赔率状态 | 详情页所有盘口、展开更多、冲突说明 |
...
重要分支场景包括：多笔单注、跨场串关、同场盘口冲突、二次确认超时、赔率变化确认、盘口暂停、开赛封盘、提前结清和历史注单重投。
...
| 盘口区 | 比赛详情页 | 展示本期 10 个盘口 | 盘口数据存在 | 用户理解并选择赔率 | 盘口关闭、隐藏、冲突 | 盘口标题、选项、赔率、状态 | 下注判断核心 | 盘口含义和状态清晰 |
| 同场互斥 | 盘口选择时 | 判断是否可组合 | 投注单已有本场投注项 | 可选或显示冲突 | 强相关盘口冲突 | 盘口分类、组合规则 | 防止强相关投注误导 | 冲突有原因和处理动作 |
| 投注单 | 详情页右侧 / 浮动入口 | 选择投注方式、复核投注项、金额、赔率并提交 | 至少 1 个投注项 | 生成多笔单注或串关注单 | 金额、余额、报价、状态不通过 | 余额、报价、投注项、限制 | 资金操作前置校验 | 成功失败都有明确反馈 |
| 二次确认 | 投注单提交 | 展示投注方式、投注项、赔率、金额和返还 | 金额和投注项合法 | 用户确认后提交 | 赔率变化、1 分钟超时、用户取消 | 报价、倒计时、投注项状态 | 防止误提交 | 弹窗内可处理赔率变化 |
...
| 更多赔率格式和接受策略说明 | 给高级用户配置赔率展示和变化接受方式 | 当前只需基础可用和可解释 |
...
flowchart TD
  ...
  amountInput --> validation[系统校验比赛、盘口、报价、金额、余额]
  validation --> confirmDialog[确认投注弹窗]
  confirmDialog --> oddsChanged{确认中是否发生赔率变化}
  oddsChanged -->|"是"| acceptOdds[用户接受当前赔率]
  oddsChanged -->|"否"| submitBet[提交投注]
  acceptOdds --> submitBet
...
| 确认超时 | 二次确认弹窗打开超过 1 分钟 | 确认按钮禁用，仅保留取消 | 用户取消后重新提交 |
| 赔率变化 | 二次确认期间任一投注项赔率变化 | 在弹窗内标记变化项 | 用户接受当前赔率后才能确认 |
...
```mermaid
flowchart TD
  existingSelection[投注单已有本场投注项] --> newMarket[用户查看另一个本场盘口]
...
| 投注单 | 下单前复核和提交 | 详情页右侧 / 浮动入口 | 输入金额、切换投注方式、提交 | 空单、多笔单注、串关、报价过期、确认超时 |
...
| 状态 | 进入条件 | 用户可做 | 禁用 / 阻塞 |
|------|----------|----------|-------------|
| 空单 | 未选择投注项 | 查看余额和设置 | 不可提交 |
| 多笔单注 | 1 个或以上投注项 | 逐项或统一输入金额、提交 | 金额或报价异常时阻塞 |
| 串关 | 2 个或以上可组合投注项 | 复核总赔率、提交 | 任一投注项异常时阻塞 |
| 报价过期 | 报价超过有效期 | 接受当前赔率 | 未接受前不可提交 |
...
|----------|----------|----------|--------------|
| 多笔单注 | 1 个或以上投注项 | 每个投注项单独计算赔率、金额和返还 | 每笔独立生成注单、独立结算 |
| 串关 | 2 个或以上跨场且可组合投注项 | 金额 × 各项赔率连乘 | 全部命中才返还 |

多笔单注允许一次提交多个投注项，但不是串关，也不合并赔率。提交成功后，每个投注项生成一笔独立注单；如果某一笔后续结算失败或退款，不影响其他单注。

所有投注方式都必须进入二次确认弹窗。二次确认弹窗打开后开始 1 分钟倒计时；1 分钟内用户未确认，则确认按钮禁用并变色，用户只能取消后重新发起确认。
...
| 注单卡片 | 注单号、状态、比赛、盘口、选择、赔率、金额、返还 | 串关可展开 |
...
| 规则 | 说明 | 用户反馈 |
|------|------|----------|
| 按钮可点击 | 比赛可下注、盘口开放、报价有效、金额合法、无冲突 | 正常提交 |
| 按钮不可点击 | 任一条件不满足 | 按原因展示禁用文案 |
| 金额校验 | 不低于最低额、不高于最高额、不超过余额、不超过返还上限 | 告诉用户调整金额 |
| 投注方式切换 | 支持多笔单注和串关两种方式 | 切换时说明规则 |
| 二次确认 | 所有提交都必须触发 | 弹窗展示投注项、金额、赔率、返还和 1 分钟倒计时 |
| 防重复提交 | 提交中禁用按钮 | 显示提交中 |
| 报价过期 | 报价有效期结束 | 要求接受当前赔率 |
| 赔率变化 | 二次确认期间当前赔率与选择时不同 | 在二次确认弹窗内要求用户接受当前赔率 |
...
| 盘口风控 | 盘口关闭、比赛不可用、强相关冲突 | 说明原因并引导移除或替换 |
...
| 报价过期 | 报价有效期结束 | 阻止提交 | 报价已过期，请重新确认当前赔率 | 是 | 可接受当前赔率 |
| 赔率变化 | 二次确认期间当前赔率与选择时不同 | 在弹窗内标记变化项 | 请接受当前赔率后继续 | 是 | 未接受前确认按钮不可用 |
| 二次确认超时 | 弹窗打开超过 1 分钟 | 禁用确认按钮 | 确认已超时，请取消后重新提交 | 是 | 只能取消 |
...
| 同场冲突 | 强相关盘口同场组合 | 阻止选择或提交 | 说明冲突对象和原因 | 是 | 可移除冲突项 |
...
| soccer_odds_select | 用户选择赔率 | 盘口、选项、赔率、是否冲突 | 选择行为 |
...
| 同场冲突 | 已选择胜平负 | 用户查看正确进球 | 展示冲突原因，不能直接组合 |
| 二次确认 | 用户点击确认投注 | 弹窗展示投注方式、明细、金额、赔率、返还和倒计时 | 用户确认后才提交 |
| 二次确认超时 | 弹窗打开超过 1 分钟 | 用户未确认 | 确认按钮禁用，只能取消 |
| 确认中赔率变化 | 二次确认期间赔率变化 | 用户查看弹窗 | 标记变化项，接受当前赔率后才能确认 |
| 开赛封盘 | 比赛进入进行中 | 用户查看盘口或提交投注单 | 所有盘口不可选，相关投注项不可提交 |
| 报价过期 | 报价有效期结束 | 用户提交投注 | 阻止提交，要求确认当前赔率 |
