...
flowchart TD
  ...
  confirmDialog --> oddsChanged{提交前是否发生赔率变化}
  oddsChanged -->|"是"| acceptOdds[用户接受最新报价]
...
| 赔率变化 | 下单区或二次确认期间任一投注项赔率变化 | 主按钮变为“接受最新报价” | 用户接受后重新核对并继续提交 |
...
多笔单注允许一次提交多个投注项，但不是串关，也不合并赔率。同一场比赛内的多个盘口可以作为多笔单注提交，因为每笔都独立生成注单、独立结算；同场强相关限制只适用于串关。

所有投注方式都必须进入二次确认弹窗。若提交前或二次确认期间发生报价变化，主按钮变为“接受最新报价”；用户接受后需要再次核对金额、赔率和可能返还，再继续确认投注。
...
| 防重复提交 | 提交中禁用按钮 | 显示提交中 |
| 报价过期 | 报价有效期结束 | 下单区和二次确认弹窗均可接受最新报价 |
| 赔率变化 | 当前赔率与选择时不同 | 主按钮变为“接受最新报价”，接受后再继续确认 |
...
| 赔率变化 | 当前赔率与选择时不同 | 主按钮变为“接受最新报价” | 请接受最新报价后继续 | 是 | 接受后需重新核对返还 |
...
| 报价变化 | 下单区或二次确认期间赔率变化 | 用户查看投注单或弹窗 | 主按钮变为“接受最新报价”，接受后继续核对提交 |
| 开赛封盘 | 比赛进入进行中 | 用户查看盘口或提交投注单 | 所有盘口不可选，相关投注项不可提交 |
| 报价过期 | 报价有效期结束 | 用户提交投注 | 主按钮变为“接受最新报价”，接受后继续核对提交 |
