...
当前要解决的问题是：足球产品已经具备首页、比赛详情、盘口、投注单、我的注单和设计状态展板，但原范围只覆盖单场比赛盘口，遗漏了冠军、晋级、赛季名次和系列赛赛果预测等赛事级盘口。v4.4 需要把单场盘口与赛事级盘口统一到同一套平台报价、投注单、二次确认和我的注单闭环中，形成可供产品评审、设计画稿、研发实现、测试验收和运营客服解释的统一 PRD。
...
| 业务目标 | 支持冠军、晋级、赛季名次和系列赛预测 | 冠军与晋级入口、赛事级详情、长期盘注单 |
| 用户目标 | 用户能理解比赛是否可下注、盘口在判断什么、为什么某些盘口不能串关 | 盘口说明、串关冲突提示、状态提示、客服解释口径 |
| 用户目标 | 用户能区分单场盘口与赛事级盘口 | 市场粒度、关闭时间、预计结算时间、官方结算来源 |
...
| 更多单场足球盘口扩展 | 单场仍按当前 10 个盘口验收；v4.4 只新增赛事级盘口 |
| 球员奖项、金靴、助攻王 | 先不进入 v4.4，避免与球员盘口扩展混淆 |
| 赛事级盘口串关 | v4.4 第一版暂不支持，可作为多笔单注提交 |
...
| 冠军与晋级用户 | 关注赛事长期结果 | 预测冠军、晋级、前四、降级或两回合系列赛赛果 | 冠军与晋级入口、赛事级详情、多笔单注 |
| 预测市场用户 | 习惯按事件问题下注 | 快速理解“这个市场何时关闭、何时结算、按什么来源结算” | 市场说明、关闭时间、预计结算、官方来源 |
...
- 单场盘口：用户进入足球首页，找到赛前比赛，进入详情页，看懂比赛状态和盘口含义，选择一个或多个开放赔率，在投注单中选择多笔单注或串关，完成二次确认后提交，提交成功后在我的注单看到注单。
- 赛事级盘口：用户进入足球首页，切换到冠军与晋级，选择赛事或赛季，查看冠军、晋级、赛季名次或系列赛预测，选择开放赔率，以多笔单注进入投注单和二次确认，提交后在我的注单看到预计结算时间和官方来源。

重要分支场景包括：多笔单注、跨场串关、串关同场盘口冲突、赛事级盘口暂不支持串关、赔率变化确认、盘口暂停、开赛封盘、市场关闭、官方结果待确认、提前结清和历史注单重投。
...
| 冠军与晋级入口 | 足球首页 | 切换到赛事级盘口 | 有赛事级市场数据 | 进入赛事级详情 | 空状态、市场暂停 | 赛事、市场、关闭时间、赔率 | 补齐长期预测主路径 | 用户能找到冠军、晋级和系列赛预测 |
| 赛事级详情页 | 冠军与晋级卡片 | 查看冠军、晋级、赛季名次和系列赛盘口 | 市场开放 | 用户可选赔率 | 市场关闭、官方待确认、作废 | 赛事级市场、结算来源 | v4.4 新增核心页 | 用户理解结算对象和时间 |
...
| 市场关闭 | 赛事级盘口达到关闭条件 | 停止接受新增选择或提交 | 关闭时间到达、阶段开始、数学确定或官方暂停 | 已选项不可提交 | 投注单含关闭市场 | 市场状态、关闭时间、官方结果 | 长期盘关键规则 | 用户知道为什么不能下注 |
...
flowchart TB
  ...
  root --> future1["赛事级路径<br/>切换冠军与晋级"]
  future1 --> future2["选择赛事、赛季或淘汰赛阶段"]
  future2 --> future3["查看冠军、晋级或系列赛预测"]
  future3 --> future4["选择开放赔率"]
  future4 --> future5["以多笔单注进入投注单"]
  future5 --> main6
  ...
  branchRoot --> branch9["赛事级盘口关闭或官方结果待确认"]
  branchRoot --> branch10["赛事级盘口暂不支持串关"]
...
赛事级盘口使用同一套投注单和二次确认能力，但选择入口、封盘条件和结算对象不同。用户不是先选择一场比赛，而是先选择赛事、赛季或淘汰赛阶段。

```mermaid
flowchart TD
  soccerHome[足球首页] --> switchFutures[切换冠军与晋级]
  switchFutures --> competitionList[查看赛事级市场列表]
  competitionList --> competitionDetail[进入赛事级详情页]
  competitionDetail --> marketGroup[查看冠军、晋级、赛季结果、系列赛分组]
  marketGroup --> chooseFutureOdds[选择开放赔率]
  chooseFutureOdds --> futureBetSlip[投注单复核]
  futureBetSlip --> betTypeCheck{是否切换串关}
  betTypeCheck -->|"是"| blockParlay[提示赛事级盘口暂不支持串关]
  blockParlay --> futureBetSlip
  betTypeCheck -->|"否"| amountInput[输入单笔金额]
  amountInput --> quoteCheck{报价是否需要确认}
  quoteCheck -->|"是"| acceptOdds[接受最新报价]
  acceptOdds --> amountInput
  quoteCheck -->|"否"| validation[校验市场状态、关闭时间、报价、金额、余额]
  validation --> confirmDialog[确认投注弹窗]
  confirmDialog --> submitBet[提交投注]
  submitBet --> success[生成赛事级单注]
  success --> myBets[我的注单展示预计结算和官方来源]
```
...
```mermaid
flowchart TD
  selectedFuture[投注单含赛事级盘口] --> statusCheck{市场是否仍开放}
  statusCheck -->|"开放"| quoteCheck{报价是否有效}
  statusCheck -->|"关闭"| blockClosed[阻止提交并提示移除不可用选项]
  statusCheck -->|"官方待确认"| blockPending[阻止新增投注，等待官方结果]
  statusCheck -->|"作废"| voidMarket[提示市场作废退款或不可提交]
  quoteCheck -->|"有效"| continueSubmit[继续二次确认]
  quoteCheck -->|"过期或变化"| acceptLatest[接受最新报价后重新复核]
```
...
|-------------|----------|----------|----------|----------|
| 足球首页 | 找到比赛和赛事级盘口 | 主导航足球入口 | 切换比赛 / 冠军与晋级、联赛筛选、进入比赛或赛事级详情 | 加载、空列表、赛前、进行中、异常 |
| 比赛详情页 | 完成下注判断 | 首页比赛卡 | 返回、查看盘口、选择赔率 | 比赛状态、盘口状态、右栏信息缺失 |
| 冠军与晋级页 | 找到赛事级盘口 | 足球首页切换、比赛详情轻入口 | 选择赛事、查看冠军 / 晋级 / 系列赛市场 | 开放、暂停、关闭、官方待确认 |
| 赛事级盘口区 | 理解并选择长期预测 | 冠军与晋级页 | 选择冠军、晋级、名次或系列赛赔率 | 开放、即将开放、暂停、已封盘、已结算 |
...
| 冠军与晋级列表 | 赛事名、地区、阶段、市场数量、热门市场 | 切换后展示 | 点击进入赛事级详情 |
...
冠军与晋级页承接赛事级盘口，不与单场比赛盘口混排。用户从足球首页切换到“冠军与晋级”后，先选择赛事、赛季或淘汰赛阶段，再进入详情页查看市场。

| 区域 | 展示字段 | 规则 |
|------|----------|------|
| 赛事卡片 | 赛事名、地区、阶段、市场数量、热门市场 | 点击进入赛事级详情 |
| 赛事头部 | 赛事名、阶段、市场粒度、报价方式、投注方式 | 明确这是赛事 / 赛季 / 系列赛预测 |
| 市场分组 | 冠军、晋级、赛季结果、系列赛 | 按业务含义分组，不按单场盘口 tab 展示 |
| 市场卡片 | 标题、选项、赔率、关闭时间、预计结算、官方来源 | 用户选择前必须知道判断对象和结算来源 |
| 规则说明 | 暂不支持串关、按官方结果结算、报价可能变化 | 常驻轻提示，不打断选择 |

v4.4 第一版赛事级盘口只支持单注和多笔单注。冠军、晋级和系列赛之间存在强相关，例如“某队夺冠”和“某队晋级决赛”并不独立；在相关性规则明确前，不允许进入串关。

赛事级盘口示例：

| 类型 | 示例 | 结算口径 |
|------|------|----------|
| 冠军 | 欧冠冠军、英超冠军、世界杯冠军 | 官方冠军或最终积分榜确认后结算 |
| 晋级 | 晋级决赛、晋级下一轮、获得欧冠资格 | 按官方晋级或资格结果结算，淘汰赛包含加时和点球 |
| 赛季结果 | 前四、前六、降级、升级 | 按官方最终积分榜和资格规则结算 |
| 系列赛 | 两回合系列赛晋级方、总比分阶段结果 | 按官方两回合结果和晋级方结算 |

#### 6.4.1 赛事级列表

赛事级列表不是比赛列表的扩展列，而是一个独立浏览模式。它解决的问题是：用户不想从某一场比赛进入，而是想直接表达对赛事长期结果的判断。

| 元素 | 展示要求 | 交互 |
|------|----------|------|
| 一级入口 | 足球首页顶部展示“比赛 / 冠军与晋级” | 默认进入比赛，用户主动切换到冠军与晋级 |
| 赛事卡片 | 赛事短名、地区、阶段、市场数量、热门市场摘要 | 点击进入赛事级详情 |
| 热门市场 | 最多展示 3 个市场摘要，例如冠军、晋级、系列赛 | 仅做入口预览，不在列表页直接下单 |
| 空状态 | 当前暂无冠军与晋级市场 | 保留回到比赛列表入口 |
| 加载状态 | 与足球首页骨架一致 | 加载完成后展示赛事卡片 |

#### 6.4.2 赛事级详情

赛事级详情页需要像比赛详情页一样承担完整解释责任：用户必须在选择前知道自己判断的是哪个赛事、哪个阶段、何时封盘、何时结算、按什么来源结算。

| 区域 | 必须展示 | 说明 |
|------|----------|------|
| 路径导航 | 足球 › 冠军与晋级 › 赛事名 | 帮助用户返回足球首页或列表 |
| 赛事头部 | 赛事名、地区、阶段、产品说明 | 明确不是单场比赛，也不是 CLOB |
| 规则摘要 | 市场粒度、报价方式、投注方式 | 当前为平台欧洲盘报价，只支持多笔单注 |
| 市场分组 | 冠军、晋级、赛季结果、系列赛 | 每组按业务含义聚合 |
| 市场说明 | 盘口标题、判断对象、结算来源 | 用户选择前可读 |
| 关闭时间 | 具体时间或“按市场规则” | 决定是否可提交 |
| 预计结算 | 决赛结束、赛季结束、次回合结束等 | 我的注单也要展示 |
| 投注单 | 与比赛详情共用右栏投注单 | 展示赛事级标签和不可串关说明 |

#### 6.4.3 赛事级市场卡片

赛事级市场卡片沿用赔率按钮组件，但标题、说明和状态语义必须区别于单场盘口。

| 字段 | 示例 | 说明 |
|------|------|------|
| 市场标题 | 欧冠冠军 | 用户判断的问题 |
| 市场分组 | 冠军 | 用于详情页分组和筛选 |
| 选项 | 皇家马德里、曼城、巴黎圣日耳曼 | 可以是球队，也可以是“是 / 否”组合 |
| 赔率 | 4.20 | 平台欧洲盘报价 |
| 关闭时间 | 05/31 03:00 | 关闭后不可新增或提交 |
| 预计结算 | 决赛结束并经 UEFA 确认后 | 注单长期待结算时的解释 |
| 结算来源 | UEFA 官方赛事结果 | 客服和用户查询依据 |
| 市场状态 | 开放、即将开放、暂停、已封盘、官方待确认、已结算、作废 | 决定可选和可提交状态 |
...
赛事级盘口不以 `matchId` 为唯一锚点，而是以赛事、赛季或淘汰赛阶段作为下注对象。产品上称为“冠军与晋级”，技术上需要保存下注对象、市场标题、选择、赔率、关闭时间、预计结算时间和官方结算来源。

v4.4 第一版规则：

| 规则 | 用户表现 | 原因 |
|------|----------|------|
| 赛事级盘口可作为多笔单注提交 | 多个冠军、晋级或系列赛选择分别生成独立单注 | 每笔独立赔率、独立金额、独立结算 |
| 赛事级盘口暂不支持串关 | 串关入口禁用或阻止切换，并提示可作为多笔单注提交 | 长期盘之间常有强相关，不能沿用同场 `matchId` 互斥 |
| 单场盘口与赛事级盘口不混排 | 首页切换入口，详情页可轻入口跳转 | 避免用户误以为冠军盘属于某一场比赛 |
| 同一赛事级盘口内只能保留一个选项 | 选择新选项时替换原选项 | 同一问题下的选项天然互斥 |

赛事级盘口状态：

| 状态 | 含义 | 用户动作 |
|------|------|----------|
| 开放 | 可选择并加入投注单 | 选择赔率、输入金额 |
| 即将开放 | 市场存在但暂未开放 | 查看说明，不可选择 |
| 暂停报价 | 平台临时停止接受投注 | 保留展示，不可提交 |
| 已封盘 | 关闭时间到达、阶段开始、数学确定或官方暂停 | 已选项不可提交 |
| 官方结果待确认 | 结果已接近确定但等待官方来源 | 不可新增投注，等待结算 |
| 已结算 | 官方结果确认 | 展示命中或未命中 |
| 作废退款 | 赛事取消或市场失效 | 按退款规则处理 |

### 7.5 v4.4 赛事级盘口清单

| 盘口 | 分类 | 用户在判断什么 | 选项形态 | 第一版状态 |
|------|------|----------------|----------|------------|
| 冠军 | 赛事优胜者 | 某赛事或联赛最终冠军是谁 | 多球队按钮 | P0 |
| 晋级 | 阶段结果 | 某队是否晋级下一轮、进入决赛或获得资格 | 是 / 否或球队按钮 | P0 |
| 赛季名次 | 赛季结果 | 某队是否前四、前六、降级或升级 | 是 / 否或球队按钮 | P0 |
| 系列赛赛果 | 淘汰赛阶段 | 两回合淘汰赛最终晋级方或阶段结果 | 晋级方 / 特殊结果按钮 | P0 |
| 球员奖项 | 个人荣誉 | 金靴、助攻王、MVP 等 | 球员列表 | 不做 |
| 组合长期盘 | 复合预测 | 冠军 + 晋级等组合 | 组合选项 | 不做 |

### 7.5.1 冠军

冠军盘口用于预测某个赛事、杯赛或联赛的最终冠军。它不是某一场比赛的胜平负，也不是某一轮淘汰赛的晋级结果。用户选择一支球队；官方确认冠军后，若所选球队为冠军则该投注项获胜，否则失败。

示例：用户选择“欧冠冠军：皇家马德里”。若 UEFA 官方确认皇家马德里获得该赛季欧冠冠军，则该投注项获胜；若皇家马德里在半决赛出局或决赛失利，则该投注项失败。

冠军盘口的赔率会随赛程、抽签、伤病、积分、淘汰赛结果和平台风险策略变化。报价变化仍按当前统一规则处理：提交前若报价变化或过期，用户必须先接受最新报价，再重新核对后提交。

冠军盘口按官方冠军结果结算。联赛冠军按官方最终积分榜和联赛公告结算；杯赛冠军按官方冠军公告或决赛结果结算。后续争议、申诉或纪律处罚是否追溯调整由后端结算规则和官方来源确认。

### 7.5.2 晋级

晋级盘口用于预测某队是否进入下一轮、进入决赛、晋级淘汰赛或获得下赛季某赛事资格。它与单场胜平负不同：单场胜平负通常只看常规比赛时间；晋级类盘口关注最终晋级事实，通常包含加时赛和点球大战。

示例：用户选择“皇家马德里晋级决赛：是”。若皇家马德里通过两回合总比分、加时赛或点球大战进入决赛，则该投注项获胜；若未进入决赛，则失败。
...
| 小组出线 | 以官方小组排名和出线规则为准 |
| 获得欧冠资格 | 以官方联赛排名和资格分配规则为准 |

晋级盘口和冠军盘口高度相关，例如“某队夺冠”必然意味着“某队进入决赛”或“某队晋级后续阶段”。因此 v4.4 第一版不允许赛事级盘口串关。

### 7.5.3 赛季名次

赛季名次盘口用于预测联赛或赛季结束后的排名、资格或升降级结果。常见示例包括：英超冠军、前四、前六、降级球队、升级球队。

示例：用户选择“纽卡斯尔获得欧冠资格：是”。若赛季结束后，纽卡斯尔按官方排名和资格分配规则获得下赛季欧冠资格，则该投注项获胜；若排名不足或资格规则不满足，则失败。

赛季名次盘口的关键不是某一场比赛，而是整个赛季的官方结果。页面必须展示预计结算时点，例如“赛季最终积分榜确认后”。若赛季延期、缩短、取消或资格规则发生变化，市场可以暂停、等待官方结果、作废或按后端结算规则处理。

### 7.5.4 系列赛赛果

系列赛赛果用于预测一个阶段性对抗的最终结果。足球 v4.4 第一版以两回合淘汰赛为主，不扩展到篮球、电竞等 BO 系列比分。后续若要支持 BO3、BO5、BO7，可沿用同一类目，但需要单独定义胜场比分和比赛取消规则。

示例：皇家马德里 vs 巴塞罗那两回合系列赛：

| 用户选择 | 命中条件 |
|----------|----------|
| 皇家马德里晋级 | 两回合结束后皇家马德里为官方晋级方 |
| 巴塞罗那晋级 | 两回合结束后巴塞罗那为官方晋级方 |
| 总比分打平后点球决胜 | 两回合总比分和适用规则导致点球大战，并通过点球产生晋级方 |

系列赛赛果不是“正确比分”。它不要求用户猜中某一场比赛的具体比分，而是猜中两回合或阶段对抗的最终结果。若未来新增“两回合总比分”或“系列赛正确比分”，应作为独立盘口，不与当前“晋级方”混写。

### 7.5.5 赛事级盘口与单场盘口的关系

| 对比项 | 单场盘口 | 赛事级盘口 |
|--------|----------|------------|
| 下注对象 | 一场比赛 | 赛事、赛季、淘汰赛阶段 |
| 入口 | 比赛列表、比赛详情 | 冠军与晋级列表、赛事级详情 |
| 封盘时点 | 比赛开赛 | 市场关闭时间、阶段开始、数学确定、官方暂停 |
| 结算依据 | 比赛赛果或盘口规则 | 官方冠军、晋级、积分榜或阶段结果 |
| 投注方式 | 多笔单注、串关 | 第一版只支持单注和多笔单注 |
| 报价变化 | 赛前报价变化、30 秒有效期 | 同样手动接受最新报价 |
| 我的注单 | 展示比赛和盘口 | 展示赛事级标签、预计结算和官方来源 |
...
赛事级市场的生命周期比单场盘口更长，且不依赖某一场比赛的开赛时间。页面和投注单必须使用“市场关闭”“官方结果待确认”等文案，不能把所有关闭状态都写成“比赛已开始，盘口已封盘”。
...
stateDiagram-v2
  ...
  Open --> Suspended: 暂停报价
  Suspended --> Open: 恢复报价
  Open --> Closed: 关闭时间到达或阶段开始
  Open --> OfficialPending: 结果数学确定或等待官方确认
...
| 状态 | 进入条件 | 用户可见 | 可选择 | 投注单处理 |
|------|----------|----------|--------|------------|
| 即将开放 | 市场已配置但未开放 | 是 | 否 | 不可加入投注单 |
| 开放 | 市场可下注 | 是 | 是 | 可提交，仍需报价有效 |
| 暂停报价 | 平台或数据源暂停 | 是 | 否 | 已选项不可提交 |
| 已封盘 | 关闭时间到达、阶段开始、数学确定 | 是 | 否 | 已选项不可提交，需要移除 |
| 官方结果待确认 | 比赛或赛季结果等待官方来源 | 是 | 否 | 不可新增，已下注等待结算 |
| 已结算 | 官方结果确认 | 是 | 否 | 注单更新返还 |
| 作废退款 | 赛事取消或市场失效 | 是 | 否 | 已下注按退款或后端规则处理 |
...
赛事级注单在“已下注”和“进行中”之间不一定能按某场比赛状态切换。第一版可统一展示为“已下注 / 待结算”，并在卡片中展示预计结算，例如“决赛结束并经 UEFA 确认后”。当官方结果确认后进入“已结算”。
...
| 下注对象 | 粒度 | match / competition / tie / season | 是 | 比赛或赛事级市场 | 投注单、注单、结算 | 区分单场、赛事、系列赛和赛季 |
| 下注对象 | 对象 ID / 名称 | 字符串 | 是 | 比赛或赛事级市场 | 投注单、注单 | 单场用比赛，长期盘用赛事或阶段 |
| 下注对象 | 关闭时间 | 时间 | 条件必填 | 赛事级市场 | 赛事级详情、投注单 | 决定是否还能新增或提交 |
| 下注对象 | 预计结算 | 文本 | 条件必填 | 赛事级市场 | 赛事级详情、注单 | 例如“决赛结束并经官方确认后” |
| 下注对象 | 结算来源 | 文本 | 条件必填 | 赛事级市场 | 赛事级详情、注单、客服 | 官方赛事结果、积分榜或晋级结果 |
...
v4.4 mock 需要至少覆盖两个赛事对象：一个杯赛 / 淘汰赛赛事，一个联赛 / 赛季对象。每个对象下至少有 2-3 个市场，覆盖冠军、晋级、赛季结果和系列赛。
...
| 字段 | 必填 | 说明 |
|------|------|------|
| `scope` | 是 | `competition`、`season`、`tie` 中之一 |
| `subjectId` | 是 | 赛事级下注对象唯一 ID，用于投注单 key |
| `subjectLabel` | 是 | 投注单、确认弹窗、我的注单展示 |
| `closesAt` | 是 | 市场关闭时间；未知时后端应给状态，不允许前端猜测 |
| `resolutionTimeLabel` | 是 | 用户可理解的预计结算时点 |
| `resolutionSource` | 是 | 官方来源，用于客服解释 |
| `group` | 是 | 冠军、晋级、赛季结果、系列赛 |
| `status` | 是 | 开放、即将开放、暂停、已封盘、官方待确认、已结算、作废 |
...
| 赛事级市场关闭 | 关闭时间到达、阶段开始、数学确定或官方暂停 | 投注项不可提交 | 该赛事级盘口已关闭，请移除相关选项 | 是 | 适用于冠军、晋级和系列赛 |
| 赛事取消或改制 | 官方取消赛事、赛季缩短或规则调整 | 暂停、作废或等待官方结果 | 该市场等待官方结果确认 | 视情况 | 以后端和官方来源为准 |
| 赛事级盘口串关 | 用户尝试将冠军、晋级或系列赛放入串关 | 阻止切换或提交 | 冠军、晋级和系列赛类盘口暂不支持串关，可作为多笔单注提交 | 是 | 避免强相关组合误导 |
...
| soccer_futures_view | 进入冠军与晋级页 | 赛事数量、市场数量、入口来源 | 赛事级入口转化 |
| soccer_future_market_view | 赛事级市场进入可视区域 | 赛事、市场类型、关闭时间、市场状态 | 长期盘关注度 |
...
| 冠军盘单注 | 用户进入冠军与晋级页 | 选择欧冠冠军赔率并提交 | 生成赛事级单注，注单展示预计结算时间和官方来源 |
| 晋级盘单注 | 用户选择晋级决赛或获得欧冠资格 | 提交投注 | 按赛事级盘口生成独立注单 |
| 系列赛预测 | 用户选择两回合系列赛晋级方 | 提交投注 | 注单说明包含加时和点球的官方晋级结果 |
| 赛事级多笔单注 | 用户选择多个冠军、晋级或系列赛市场 | 选择多笔单注提交 | 生成多笔独立注单，不合并赔率 |
| 赛事级盘口暂不串关 | 投注单含冠军或晋级类盘口 | 用户切换串关 | 阻止切换并提示可作为多笔单注提交 |
| 赛事级市场关闭 | 市场关闭时间到达或官方暂停 | 用户提交投注单 | 阻止提交并提示移除不可用选项 |
...
| 场景 | Given | When | Then |
|------|-------|------|------|
| 冠军与晋级入口 | 足球首页有赛事级市场 | 用户点击“冠军与晋级” | 首页主区切换为赛事级列表，不影响比赛列表筛选 |
| 赛事级详情 | 用户点击欧冠赛事卡 | 进入详情页 | 展示赛事头部、阶段、市场粒度、报价方式和投注方式 |
| 冠军市场展示 | 欧冠冠军市场开放 | 用户查看市场卡 | 展示选项、赔率、关闭时间、预计结算、官方来源 |
| 晋级市场展示 | 晋级决赛市场开放 | 用户查看说明 | 文案说明包含加时和点球后的官方晋级结果 |
| 系列赛市场展示 | 两回合系列赛市场开放 | 用户查看说明 | 文案说明不是单场比分，而是两回合最终晋级方或阶段结果 |
| 赛事级选择 | 市场开放 | 用户选择球队或选项 | 投注单新增赛事级投注项，并显示赛事级标签 |
| 同市场替换 | 已选欧冠冠军皇家马德里 | 用户选择曼城 | 投注单替换原冠军选项，不新增第二条同市场选项 |
| 多笔赛事级单注 | 用户选择冠军、晋级、系列赛多个市场 | 输入金额并提交 | 每个投注项独立生成单腿注单 |
| 阻止赛事级串关 | 投注单含冠军或晋级市场 | 用户切换串关 | 阻止切换，并提示“冠军、晋级和系列赛类盘口暂不支持串关” |
| 报价变化 | 赛事级赔率变化 | 用户提交 | 主按钮变为“接受最新报价”，接受后继续复核 |
| 关闭时间到达 | 市场已封盘 | 用户尝试提交已选项 | 阻止提交，提示该赛事级盘口已关闭 |
| 官方待确认 | 阶段结果等待官方确认 | 用户查看市场 | 市场不可新增投注，注单等待结算 |
| 作废退款 | 赛事取消或市场作废 | 用户查看注单 | 注单展示作废或退款结果，以后端结算为准 |
| 重投赛事级注单 | 历史赛事级注单仍开放 | 用户点击重投 | 回填投注单并要求重新确认报价 |
| 重投已关闭市场 | 历史赛事级市场已封盘 | 用户点击重投 | 提示该市场已关闭，不能重投 |
...
| 赛事级盘口 | 围绕赛事、赛季或淘汰赛阶段的盘口，例如冠军、晋级、前四、降级和两回合系列赛 |
| 冠军与晋级 | v4.4 用户入口名称，承载赛事级盘口 |
| 系列赛预测 | 预测两回合淘汰赛或后续 BO 系列的最终结果，足球第一版以两回合晋级方为主 |
| 关闭时间 | 赛事级盘口停止接受新增选择或提交的时间 |
| 预计结算 | 用户可理解的结算时点，例如“决赛结束并经 UEFA 确认后” |
| 官方来源 | 结算依据来源，例如 UEFA 官方结果或 Premier League 官方积分榜 |
