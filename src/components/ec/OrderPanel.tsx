...
export default function OrderPanel() {
  ...
  function handleBet(direction: 'higher' | 'lower') {
    if (amount < EC_MIN_BET) {
      addToast({ type: 'error', message: `Minimum bet is $${EC_MIN_BET}` })
      return
    }
    if (amount > balance) {
    ...
    if (bet) {
      addToast({
        type: 'success',
        message: `${direction === 'higher' ? 'Higher' : 'Lower'} — $${amount} placed`,
      })
    }
  }
    ...
    <section className="mb-4 bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-4">
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        <div className="text-center">
          <div className="text-[10px] text-[var(--text-tertiary)] mb-0.5">Higher Profit</div>
          <div className="text-sm font-bold text-[#2DD4BF] tabular-nums">
            +${higherProfit.toFixed(2)}
          </div>
          <div className="text-[10px] text-[var(--text-tertiary)]">
            {(odds.higher * 100).toFixed(0)}% return
          </div>
        </div>
        <div className="text-center">
          <div className="text-[10px] text-[var(--text-tertiary)] mb-0.5">Lower Profit</div>
          <div className="text-sm font-bold text-[#F87171] tabular-nums">
            +${lowerProfit.toFixed(2)}
          </div>
          <div className="text-[10px] text-[var(--text-tertiary)]">
            {(odds.lower * 100).toFixed(0)}% return
          </div>
        </div>
      </div>
      ...
      <div className="text-center text-[10px] text-[var(--text-tertiary)] mb-3">
        Loss if wrong: -${amount.toFixed(2)}
      </div>
