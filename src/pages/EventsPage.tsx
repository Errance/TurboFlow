...
function EventCard({ event }: { event: PredictionEvent }) {
  ...
  return (
    <Card hover onClick={() => navigate(`/event/${event.id}`)}>
      ...
      {/* Title + description */}
      <h2 className="text-base font-semibold text-[var(--text-primary)] line-clamp-2 mb-1">{event.title}</h2>
...
function CompactEventRow({ event, badge }: { event: PredictionEvent; badge?: React.ReactNode }) {
  ...
  return (
    <div onClick={() => navigate(`/event/${event.id}`)}
      className="flex items-center gap-3 py-2.5 px-3 rounded-lg hover:bg-[var(--bg-control)] cursor-pointer transition-colors group">
      <div className="flex-1 min-w-0">
        <p className="text-xs text-[var(--text-primary)] font-medium line-clamp-1 group-hover:text-[#2DD4BF] transition-colors">{event.title}</p>
        <div className="flex items-center gap-2 mt-0.5">
          <span className="text-[10px] text-[var(--text-secondary)]">{event.category}</span>
          <span className="text-[10px] text-[var(--text-secondary)] font-mono">{formatVolume(event.totalVolume)}</span>
        </div>
      </div>
        ...
        <div className="flex justify-center gap-1.5 mt-3">
          {slides.map((_, i) => (
            <button key={i} onClick={() => setCurrent(i)}
              className={`w-1.5 h-1.5 rounded-full transition-colors ${i === current ? 'bg-[#2DD4BF]' : 'bg-[var(--border)]'}`} />
          ))}
        </div>
      )}
    </div>
    ...
    <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-4">
      ...
      <div className="divide-y divide-[var(--border)]">
        {trending.map((event, i) => (
          <CompactEventRow key={event.id} event={event} badge={<span className="text-[10px] text-[var(--text-secondary)] font-mono">#{i + 1}</span>} />
        ))}
      </div>
    </div>
  )
      ...
      <div className="space-y-1">
        {movers.map(({ event, contract }) => (
          <div key={contract.id} onClick={() => navigate(`/event/${event.id}`)}
            className="flex items-center gap-3 py-2 px-3 rounded-lg hover:bg-[var(--bg-control)] cursor-pointer transition-colors">
            <div className="flex-1 min-w-0">
              <p className="text-xs text-[var(--text-primary)] font-medium line-clamp-1">{contract.label}</p>
              <p className="text-[10px] text-[var(--text-secondary)] line-clamp-1">{event.title}</p>
            </div>
            <div className="text-right shrink-0">
              <p className="text-xs font-mono font-medium text-[var(--text-primary)]">{contract.probability}%</p>
            ...
            {tagCounts.map(([tag, count]) => (
              <button key={tag} onClick={() => setTagFilter(tag)}
                className={`w-full flex items-center justify-between px-3 py-1.5 rounded-lg text-xs transition-colors ${
                  tagFilter === tag ? 'bg-[#2DD4BF]/10 text-[#2DD4BF] font-medium' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--border)]'
                }`}>
          ...
          {assetClasses.map((cls) => (
            <button key={cls} onClick={() => setAssetClassFilter(cls)}
              className={`w-full flex items-center justify-between px-3 py-1.5 rounded-lg text-sm transition-colors ${
                assetClassFilter === cls ? 'bg-[#2DD4BF]/10 text-[#2DD4BF] font-medium' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--border)]'
              }`}>
        ...
        <>
          ...
          {sportFilter !== 'All' && (
            <div className="flex items-center gap-1.5 mb-4 text-xs">
              ...
              {leagueFilter !== 'All' && (<><span className="text-[var(--border)]">/</span><span className="text-[#2DD4BF]">{leagueFilter}</span></>)}
            </div>
          )}
          {sportsFiltered.length === 0 ? (
            ...
            <div className="space-y-6">
              {sportsGroupedByDate.map(([date, events]) => (
                <div key={date}>
                  <h2 className="text-xs text-[var(--text-secondary)] font-medium uppercase tracking-wider mb-3 px-1">{date}</h2>
        ...
        <div className="flex items-center gap-2">
          {!isLanding && (
            <button onClick={() => setMobileFilterOpen(true)}
              className="md:hidden min-w-[44px] min-h-[44px] flex items-center justify-center text-[var(--text-secondary)] hover:text-[var(--text-primary)] rounded-lg hover:bg-[var(--border)] transition-colors">
        ...
        return (
          <>
            ...
            <div className="hidden md:flex fixed inset-0 z-40 items-center justify-center">
              ...
              <div className="relative z-50 w-[380px] max-h-[85vh] overflow-y-auto bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl shadow-2xl">
                <div className="flex items-center justify-between px-4 pt-3 pb-0">
                  <span className="text-[10px] text-[var(--text-secondary)] uppercase tracking-wider">Quick Trade</span>
                  <button onClick={closeTradePanel} className="min-w-[32px] min-h-[32px] flex items-center justify-center text-[var(--text-secondary)] hover:text-[var(--text-primary)] rounded-lg hover:bg-[var(--border)]">
