          ...
          return (
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-[#8A8A9A] font-mono">{formatVolume(c.volume)} vol</p>
              </div>
        ...
        <div className="flex-1">
          ...
          <div className="bg-[#161622] border border-[#252536] rounded-xl p-5 mb-4">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-xs text-[#8A8A9A]">{sports.league}</span>
            ...
            <div className="flex items-center justify-between">
              <div className="flex-1 space-y-3">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-[#252536] flex items-center justify-center text-sm font-bold text-white">
                    {sports.awayTeam.abbreviation}
                  </div>
                  <div>
                    <p className="text-base font-semibold text-white">{sports.awayTeam.name}</p>
                    {sports.awayTeam.record && (
                      <p className="text-xs text-[#8A8A9A]">{sports.awayTeam.record}</p>
                    )}
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-base font-semibold text-white">{sports.homeTeam.name}</p>
                    {sports.homeTeam.record && (
                      <p className="text-xs text-[#8A8A9A]">{sports.homeTeam.record}</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              {sports.score && (
                <div className="text-right space-y-3">
                  <p className="text-2xl font-bold font-mono text-white">{sports.score.away}</p>
                  <p className="text-2xl font-bold font-mono text-white">{sports.score.home}</p>
                </div>
              )}
            </div>

            {/* Volume */}
            <div className="mt-4 pt-3 border-t border-[#252536] flex items-center gap-4 text-xs text-[#8A8A9A]">
          ...
          {isCancelled && (
            <div className="bg-[#252536] border border-[#252536] rounded-xl p-4 mb-4">
          ...
          <details className="mt-4">
            <summary className="text-xs text-[#8A8A9A] cursor-pointer hover:text-white transition-colors py-2">
              Rules & Settlement
            </summary>
            <div className="bg-[#161622] border border-[#252536] rounded-xl p-4 mt-1 space-y-2">
              <div className="flex gap-2">
                <span className="text-xs text-[#8A8A9A] shrink-0 w-20">Measurement</span>
                <span className="text-xs text-white">{event.rulesMeasurement}</span>
              </div>
              <div className="flex gap-2">
                <span className="text-xs text-[#8A8A9A] shrink-0 w-20">Closing</span>
                <span className="text-xs text-white">{event.rulesClosing}</span>
              </div>
              <div className="flex gap-2">
                <span className="text-xs text-[#8A8A9A] shrink-0 w-20">Source</span>
                <span className="text-xs text-white">{event.resolutionSource}</span>
              </div>
              {event.rulesDetail && (
                <p className="text-xs text-[#8A8A9A] mt-2 whitespace-pre-line">{event.rulesDetail}</p>
              )}
            </div>
          </details>
        </div>
