...
import { clobMatches, leagues } from '../data/clob/mockData'
import ClobMatchListCard from '../components/clob/ClobMatchListCard'
...
export default function ClobPage() {
  ...
  const filteredMatches = selectedLeague === 'all'
    ? clobMatches
    : clobMatches.filter((m) => m.leagueId === selectedLeague)

  const liveCount = clobMatches.filter((m) => m.status === 'live').length
  const groupedByLeague = filteredMatches.reduce<Record<string, typeof clobMatches>>((acc, m) => {
    ;(acc[m.league] ??= []).push(m)
    return acc
  }, {})
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <nav className="md:w-56 shrink-0">
          ...
          <div className="space-y-0.5">
            ...
            {leagues.map((league) => {
              const leagueMatches = clobMatches.filter((m) => m.leagueId === league.id)
            ...
            <div key={league} className="mb-6">
              ...
              <div className="border border-[var(--border)] rounded-lg overflow-hidden">
                {leagueMatches.map((match) => (
                  <ClobMatchListCard key={match.id} match={match} />
                ))}
              </div>
            </div>
          ))}
